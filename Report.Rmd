...
After resampling we now have `r sum(re_seimic$class==1)` positive class and `r sum(re_seimic$class==0)` negative class. The data set now becomes balance.

```{r, echo=TRUE}
sum(re_seimic$class==1)
```

```{r, echo=TRUE}
sum(re_seimic$class==0)
```

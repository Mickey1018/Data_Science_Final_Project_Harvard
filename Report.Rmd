---
title: "Prediction on Seismic Hazard in Mining Work with Machine Learning Methods"
author: "FUNG CHE HEI"
date: "8/28/2020"
output:
  pdf_document:
...
In the following sections, data cleaning and data exploration would be performed first to analysis the effect on output by different features. Then it will follow by building several classification models  by using different machine learning methods. Finally the models will be evaluated and compared.  
...
```{r, echo=FALSE, figures-side, fig.show="hold", out.width="50%", fig.cap="seismic distribution on positive class and negative class"}
#extract the index of incorrect data set
incorrect_index<- 
  seismic %>% 
  mutate(total = nbumps2+nbumps3+nbumps4+nbumps5+nbumps6+nbumps7+nbumps89) %>%
...
```{r, echo=FALSE, fig.show="hold", out.width="50%", fig.cap="seismoacoustic distribution on positive class and negative class"}
corrected_seismic %>%
  filter(class==1) %>%
...
```{r, echo=FALSE, fig.show="hold", out.width="50%", fig.cap="shift type distribution on positive class and negative class"}
corrected_seismic %>%
  filter(class==1) %>%
  ggplot(aes(shift)) +
  geom_bar(width = 0.1,fill="red",col="black") +
...
```{r, fig.cap="genergy in positive class and negative class"}
corrected_seismic %>% 
...
```{r, fig.cap="gdenergy in positive class and negative class"}
corrected_seismic %>% 
...
```{r, fig.cap="gpuls distribution on positive class and negative class"}
...
```{r, fig.cap="gdpuls distribution on positive class and negative class"}
mu_gdpuls <- 
...
```{r, echo=FALSE, fig.show="hold", out.width="50%", fig.cap="ghazard distribution on positive class and negative class"}
corrected_seismic %>%
  filter(class==1) %>%
  ggplot(aes(ghazard)) +
  geom_bar(width = 0.1,fill="red",col="black") +
...
```{r, echo=FALSE, fig.show="hold", out.width="50%", fig.cap="number of seismic bumps with different energy range on positive class and negative class"}
...
```{r, echo=FALSE, fig.cap="total energy of seismic bumps on positive class and negative class"}
...
```{r,echo=FALSE, fig.cap="maximum energy of seismic bumps on positive class and negative class"}
...
```{r, echo=TRUE, fig.cap = "number of nearest neighbors v.s. accuracy in repeated cross-validation"}
ggplot(fit_knn)
```
...
```{r, echo = TRUE, fig.cap = "complexity parameter v.s. accuracy in repeated cross-validation"}
ggplot(fit_rpart)
```
...
```{r, echo = TRUE, fig.cap = "mtry v.s. accuracy in repeated cross-validation"}
ggplot(fit_rf)
```
...
```{r, echo = TRUE, warning=FALSE, fig.cap = "ROC Curve for KNN"}
...
```{r, echo = TRUE, warning=FALSE, fig.cap = "ROC Curve for Decision Tree"}
...
```{r, echo = TRUE, warning = FALSE, fig.cap = "ROC Curve for Random Forest"}
...
```{r, echo = FALSE}
kable(data.frame(Model = c("KNN", "Decision Tree", "Random Forest"),
                 Sensitivity = c(pROC_knn$sensitivities[2],
                                 pROC_rpart$sensitivities[2],
                                 pROC_rf$sensitivities[2]),
                 AUC = c(pROC_knn$auc,
                         pROC_rpart$auc,
                         pROC_rf$auc)), caption = "Sensitivity and AUC in different models")
```

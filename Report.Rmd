...
We have now `r nrow(re_seismic_train)` training data and `r nrow(re_seismic_test)` test data. In the following sections, we would try different training methods and see which perform the best.

During the training process, Repeated k-fold Cross Validation will be used. This is the repeating process of splitting the data into k-folds, and the final model accuracy is calculated as the mean from the numbers of repeats. For the following traiing, 10-fold cross validation with 3 repeats will be used for our data set.
